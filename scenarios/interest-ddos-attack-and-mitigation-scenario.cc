...
void PrintTime(Time next, const string name) {
    cerr << " === " << name << " " << Simulator::Now().ToDouble(Time::S) << "s"
         << endl;
...
int main(int argc, char **argv) {
    ...
    if (prefix == "simple-limits") {
        ...
        helper.SetForwardingStrategy(
            "ns3::ndn::fw::BestRoute::PerOutFaceLimits", "Limit",
        ...
        helper.EnableLimits(true, defaultRtt);
        helper.SetForwardingStrategy(
            "ns3::ndn::fw::BestRoute::Stats::"
            "SatisfactionBasedInterestAcceptance::PerOutFaceLimits",
            "GraceThreshold", "0.05");
        ...
        helper.SetForwardingStrategy("ns3::ndn::fw::Flooding");
    } else {
    ...
    if (leaves.GetN() < goodCount + badCount) {
        NS_FATAL_ERROR(
            "Number of good and bad guys ("
            << (goodCount + badCount)
            << ") cannot be less than number of leaves in the topology ("
            << leaves.GetN() << ")");
        exit(1);
    }
    ...
    if (producerLocation == "gw") {
        if (gw.GetN() < 1) {
            NS_FATAL_ERROR(
                "Topology does not have gateway nodes that can serve as "
                "producers");
            exit(1);
        }
        ...
        if (bb.GetN() < 1) {
            NS_FATAL_ERROR(
                "Topology does not have backbone nodes that can serve as "
                "producers");
            exit(1);
        }
    } else {
    ...
    while (evils.size() < badCount) {
        ...
        if (evils.find(node) != evils.end()) continue;
        evils.insert(node);
    ...
    std::for_each(producers.begin(), producers.end(),
                  assignNodes(producerNodes, "Producers"));
    os << "\n";
    ...
    for (NodeList::Iterator node = NodeList::Begin(); node != NodeList::End();
        ...
        if (2 * fib->Begin()->m_faces.begin()->GetRealDelay().ToDouble(
                    Time::S) >
            defaultRtt.ToDouble(Time::S)) {
            cout << "DefaultRTT is smaller that real RTT in the topology: "
                 << 2 * fib->Begin()->m_faces.begin()->GetRealDelay().ToDouble(
                            Time::S) << "s" << endl;
            os << "DefaultRTT is smaller that real RTT in the topology: "
               << 2 * fib->Begin()->m_faces.begin()->GetRealDelay().ToDouble(
                          Time::S) << "s" << endl;
        }
    }

    double maxNonCongestionShare =
        0.8 * calculateNonCongestionFlows(goodNodes, producerNodes);
    os << "maxNonCongestionShare   " << maxNonCongestionShare << endl;

    saveActualGraph(graph_dot_file, NodeContainer(goodNodes, evilNodes));
    system(("twopi -Tpdf \"" + graph_dot_file + "\" > \"" + graph_pdf_file +
            "\"").c_str());
    ...
    for (NodeContainer::Iterator node = goodNodes.Begin();
        ...
        goodApp.Start(Seconds(0.0) +
                      Time::FromDouble(rand.GetValue(), Time::S));
    }

    for (NodeContainer::Iterator node = evilNodes.Begin();
         node != evilNodes.End(); node++) {
        ApplicationContainer evilApp;
        evilAppHelper.SetPrefix("/evil/" + Names::FindName(*node));
        evilApp.Add(evilAppHelper.Install(*node));

        UniformVariable rand(0, 1);
        evilApp.Start(Seconds(25.0) +
                      Time::FromDouble(rand.GetValue(), Time::MS));
        evilApp.Stop(Seconds(125.0) +
                     Time::FromDouble(rand.GetValue(), Time::MS));
    }
