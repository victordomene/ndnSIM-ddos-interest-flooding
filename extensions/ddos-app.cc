...
TypeId DdosApp::GetTypeId() {
    static TypeId tid =
        TypeId("DdosApp")
            .SetParent<App>()
            .AddConstructor<DdosApp>()
            .AddAttribute("AvgGap", "AverageGap", StringValue("1ms"),
                          MakeTimeAccessor(&DdosApp::m_avgGap),
                          MakeTimeChecker())
            .AddAttribute("Prefix", "Name of the Interest", StringValue("/"),
                          MakeNameAccessor(&DdosApp::m_prefix),
                          MakeNameChecker())
            .AddAttribute("LifeTime", "Interest lifetime", StringValue("1s"),
                          MakeTimeAccessor(&DdosApp::m_lifetime),
                          MakeTimeChecker())
            .AddAttribute("Evil", "Evil bit", BooleanValue(false),
                          MakeBooleanAccessor(&DdosApp::m_evilBit),
                          MakeBooleanChecker())
            .AddAttribute("DataBasedLimits",
                          "Calculate frequency based on how many data packets "
                          "can be returned",
                          BooleanValue(true),
...
void DdosApp::OnNack(Ptr<const Interest> interest) {
    ...
    std::cout << "Nack received: " << interest->GetName() <<  std::endl;
}
...
void DdosApp::SendPacket() {
    ...
    m_nextSendEvent = Simulator::Schedule(nextTime, &DdosApp::SendPacket, this);
}

void DdosApp::StartApplication() {
    ...
    if (m_evilBit) {
        ...
        // m_avgGap = Seconds (2 * 1 / maxDataBack); // request 50% of maximum
        // link capacity
...
void DdosApp::StopApplication() {
    m_nextSendEvent.Cancel();

    // std::cerr << "# references before delayed stop: " <<
    // m_face->GetReferenceCount () << std::endl;
    Simulator::Schedule(Seconds(10.0), &DdosApp::DelayedStop, this);
}

void DdosApp::DelayedStop() {
    // std::cerr << "# references after delayed stop: " <<
    // m_face->GetReferenceCount () << std::endl;
    App::StopApplication();
}
