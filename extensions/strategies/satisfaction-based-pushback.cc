...
LogComponent SatisfactionBasedPushback<Parent>::g_log =
    LogComponent(SatisfactionBasedPushback<Parent>::GetLogName().c_str());

template <class Parent>
TypeId SatisfactionBasedPushback<Parent>::GetTypeId(void) {
    static TypeId tid =
        TypeId((super::GetTypeId().GetName() + "::SatisfactionBasedPushback")
                   .c_str())
            .SetGroupName("Ndn")
            ...
            .template AddAttribute(
                 "GraceThreshold",
                 "Fraction of resources that we are willing to sacrifice for "
                 "\"bad traffic\"",
                 DoubleValue(0.05),
                 MakeDoubleAccessor(
                     &SatisfactionBasedPushback::m_graceThreshold),
                 MakeDoubleChecker<double>())
            .template AddTraceSource(
                 "OnLimitAnnounce", "Event called when limits are announced",
                 MakeTraceSourceAccessor(
                     &SatisfactionBasedPushback::m_onLimitsAnnounce));
    return tid;
}

template <class Parent>
void SatisfactionBasedPushback<Parent>::DoDispose() {
    super::DoDispose();
}

template <class Parent>
void SatisfactionBasedPushback<Parent>::NotifyNewAggregate() {
    super::NotifyNewAggregate();

    if (!m_announceEventScheduled) {
        if (this->m_pit != 0 && this->m_fib != 0 &&
            this->template GetObject<Node>() != 0) {
            m_announceEventScheduled = true;
            UniformVariable r(0, 1);
            Simulator::ScheduleWithContext(
                this->template GetObject<Node>()->GetId(),
                Seconds(r.GetValue()),
                &SatisfactionBasedPushback<Parent>::AnnounceLimits, this);
        }
    }
}

template <class Parent>
std::string SatisfactionBasedPushback<Parent>::GetLogName() {
    return super::GetLogName() + ".SatisfactionBasedPushback";
}

template <class Parent>
void SatisfactionBasedPushback<Parent>::AddFace(Ptr<Face> face) {
...
template <class Parent>
void SatisfactionBasedPushback<Parent>::OnInterest(Ptr<Face> face,
...
template <class Parent>
void SatisfactionBasedPushback<Parent>::AnnounceLimits() {
    ...
    for (uint32_t faceId = 0; faceId < l3->GetNFaces(); faceId++) {
        ...
        ndnSIM::LoadStatsNode::stats_container::const_iterator item =
            stats.incoming().find(inFace);
        ...
        if (countAbs < 0.001) {
            continue;  // don't count face if it doesn't have incoming traffic.
                       // weight will be 1 anyways
            countAbs = 1.0;
        }
        ...
        double weight =
            1.00 - std::min(1.00 - m_graceThreshold, unsatisfiedAbs / countAbs);
    ...
    for (Ptr<fib::Entry> entry = this->m_fib->Begin();
         entry != this->m_fib->End(); entry = this->m_fib->Next(entry)) {
        Ptr<Interest> announceInterest = Create<Interest>();
        announceInterest->SetScope(0);  // link-local

        double totalAllowance = 0.0;
        bool unlimited = false;
        for (fib::FaceMetricContainer::type::iterator fibFace =
                 entry->m_faces.begin();
             fibFace != entry->m_faces.end(); fibFace++) {
            ...
            totalAllowance +=
                fibFace->GetFace()->GetObject<Limits>()->GetCurrentLimit();
            // totalAllowance += fibFace->m_face->GetObject<Limits>
            // ()->GetMaxLimit ();
        }
        ...
        for (uint32_t faceId = 0; faceId < l3->GetNFaces(); faceId++) {
            ...
            if (countAbs > 0.001) {
                if (unsatisfiedAbs < 0) {
                    // std::cerr << "This could be a problem with another
                    // solution\n" << std::endl;
                    // may be do something else
                    unsatisfiedAbs = 0;
                }

                weight = 1.00 - std::min(1.00 - m_graceThreshold,
                                         unsatisfiedAbs / countAbs);
            ...
            // std::cout << Simulator::Now ().ToDouble(Time::S) << "s" << "\t"
            // << Simulator::GetContext () << "\t" << weight << "\t" <<
            // realWeight
            //           << "\tunsat:" << unsatisfiedAbs << "\tcount:" <<
            // countAbs << std::endl;

            weight =
                static_cast<double>(std::max(0.0, weight * totalAllowance));

            Ptr<NameComponents> prefixWithLimit =
                Create<NameComponents>(entry->GetPrefix());
            prefixWithLimit->append("limit")
            ...
            m_onLimitsAnnounce(inFace, realWeight, weight / totalAllowance,
                               weight);
        }
    }
    ...
    Simulator::Schedule(Seconds(1.0),
                        &SatisfactionBasedPushback::AnnounceLimits, this);
}

template <class Parent>
void SatisfactionBasedPushback<Parent>::ApplyAnnouncedLimit(
...
typedef PerOutFaceLimits<SatisfactionBasedPushback<BestRoute> >
    PerOutFaceLimitsSatisfactionBasedPushbackBestRoute;
